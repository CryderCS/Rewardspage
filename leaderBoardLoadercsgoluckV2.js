...
const spreadsheetId = '1iqA0_kdkjt_DQhFcDF_MWwY6mLTmxkfMX8HJKC9-S80';
const range = 'Leaderboard!A1:G100';
    ...
    gapi.client.sheets.spreadsheets.values.get({
        ...
        if (data.length > 0) {
            ...
            let filteredData = data.slice(1).map(row => [
                row[0],  // Rang
                row[2],  // Name
                parseFloat(row[3]) || 0,  // wagered
                row[4],  // Price
                row[6],  // Enddate (wird später für Countdown benötigt)
            ]);
            ...
            startCountdown(filteredData[0][4]); // Annahme: Enddatum ist in der ersten Spalte (A) der ersten Zeile (1)
            gapi.client.sheets.spreadsheets.values.get({
...
function displayTopThreeInBoxes(topThree) {
    const loader = document.getElementById('loader-csgoluck');
    const container = document.getElementById('top-tiles-csgoluck');
    ...
    topThree.forEach((player, index) => {
        ...
        box.innerHTML = `
            ...
            <div class="info-value">
                <span class="csgoluck-icon-small"></span>
                 ${player[2]}
            </div>
            <div class="info-label">PRIZE</div>
            <div class="prize">
                <span class="csgoluck-icon-big"></span> ${player[3]}
            </div>
...
function displayAllParticipants(participants) {
    const table = document.getElementById('sheet-table-csgoluck');
    ...
    participants.forEach((row, index) => {
        ...
        tr.innerHTML = `
            ...
            alt="Alternative image"/>${row[1]}</td>
                <td><img class="coin" src="/images/csgoluckCoin.png" /> ${row[2]}
                </td>
                <td><img class="coin" src="/images/csgoluckCoin.png" /> ${row[3]}
                </td>
        `;
...
function startCountdown(targetDate) {
    const countdownElement = document.getElementById("countdown-csgoluck");
