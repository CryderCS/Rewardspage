...
const spreadsheetId = '1kRcBIIKgUq-VqCRnc6kJo9uNlaeERGj7nBaPLDV2Qes';
const range = 'Sheet1';
...
function getSheetData() {
    ...
    gapi.client.sheets.spreadsheets.values.get({
        ...
        if (data.length > 0) {
            ...
            filteredData = filteredData.filter(row => row[3] !== 0);
           ...
           console.log("Gefundenes Enddatum Skinrave:", filteredData[0][7]);
            if (filteredData.length > 0 && filteredData[0][7]) {
                console.log("Gefundenes Enddatum Skinrave:", filteredData[0][7]); // Debugging
                startCountdown(filteredData[0][7]);  // Das Enddatum von Platz 1 verwenden
            } else {
...
function displayTopThreeInBoxes(topThree) {
    const loader = document.getElementById('loader-skinrave');
    const container = document.getElementById('top-tiles-skinrave');
    ...
    topThree.forEach((player, index) => {
        ...
        box.innerHTML = `
            <img src="${player[6]}" onerror="this.onerror=null; this.src='images/CryderLogoSpin.gif';" 
            alt="Alternative image"/>
            <div class="player-name">${player[2]}</div>
            <div class="info-label">WAGERED</div>
            <div class="info-value">
            <span class="skinrave-icon-small"></span>
                 ${player[3]}
            </div>
            <div class="info-label">PRIZE</div>
            <div class="prize">
            <span class="skinrave-icon-big"></span>
                 ${player[5]}
            </div>
...
function displayAllParticipants(participants) {
    const table = document.getElementById('sheet-table-skinrave');
    ...
    participants.forEach((row, index) => {
        ...
        tr.innerHTML = `
                ...
                <td><img class="table-avatar" src="${row[6]}" onerror="this.onerror=null; this.src='images/CryderLogoSpin.gif';" 
            alt="Alternative image"/>${row[2]}</td>
                <td><img class="coin" src="/images/skinravecoin.svg" />
                ${row[3]}
                </td>
                <td><img class="coin" src="/images/skinravecoin.svg" />
                ${row[5]}
...
function startCountdown(targetDate) {
    const countdownElement = document.getElementById("countdown-skinrave");
