...
const spreadsheetId = '1qNAA5yFck9552b7Z1Fo_pN_OcfMee3XSIOh-VGkdd4g';
...
function getSheetData() {
    ...
    gapi.client.sheets.spreadsheets.values.get({
        ...
        if (data.length > 0) {
           ...
           console.log("Gefundenes Enddatum CSGOGEM:", filteredData[0][5]);
            if (filteredData.length > 0 && filteredData[0][5]) {
                console.log("Gefundenes Enddatum CSGOGEM:", filteredData[0][5]); // Debugging
...
function displayTopThreeInBoxes(topThree) {
    const loader = document.getElementById('loader-csgogem');
    const container = document.getElementById('top-tiles-csgogem');
    ...
    topThree.forEach((player, index) => {
        ...
        box.innerHTML = `
            ...
            <span class="csgogem-icon-small"></span>
                 ${player[3]}
            </div>
            <div class="info-label">PRIZE</div>
            <div class="prize">
            <span class="csgogem-icon-big"></span>
                 ${player[1]}
            </div>
...
function displayAllParticipants(participants) {
    const table = document.getElementById('sheet-table-csgogem');
    ...
    participants.forEach((row, index) => {
        ...
        tr.innerHTML = `
            ...
            alt="Alternative image"/>${row[2]}</td>
                <td><img class="coin" src="/images/gemcoin.avif" />
                ${row[3]}
                </td>
                <td><img class="coin" src="/images/gemcoin.avif" />
                ${row[1]}
...
function startCountdown(targetDate) {
    const countdownElement = document.getElementById("countdown-csgogem");
