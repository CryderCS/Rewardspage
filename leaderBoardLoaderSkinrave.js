...
const spreadsheetId = '1kRcBIIKgUq-VqCRnc6kJo9uNlaeERGj7nBaPLDV2Qes'; // Dein Spreadsheet-ID (ersetze dies)
const range = 'Sheet1!A1:L100'; // Bereich in deinem Google Sheet
    ...
    setTimeout(() => {
        gapi.client.sheets.spreadsheets.values.get({
            ...
            if (data.length > 0) {
                ...
                const filteredData = data.slice(0).map(row => [
                    row[2],  // Spalte C Name
                    row[0],  // Spalte A Rang
                    row[11],  // Spalte L Price
                    row[7],   // Spalte G wagered
                    row[10]     //Avatar
                ]);
...
function displayTopThreeInBoxes(topThree) {
    ...
    topThree.forEach((row, index) => {
        ...
        box.classList.add('participant-box-skinrave');
        ...
        box.innerHTML = `
            ...
            <p>Prize:</p>
            <p><img src="/images/ravecoin.png" alt="Rollcoin" style="width: 19px; height: 19px"><strong>${row[2]}</strong> </p>
            <p>Wagered:</p>
