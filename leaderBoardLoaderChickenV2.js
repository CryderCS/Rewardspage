...
const spreadsheetId = '1jkyYn2wcwHcmLh_JsTeLLhB24SdMM44u-woMtYA6qOo';
...
function getSheetData() {
    ...
    gapi.client.sheets.spreadsheets.values.get({
        ...
        if (data.length > 0) {
           ...
           console.log("Gefundenes Enddatum Chicken:", filteredData[0][6]);
            if (filteredData.length > 0 && filteredData[0][6]) {
                console.log("Gefundenes Enddatum Chicken:", filteredData[0][6]); // Debugging
...
function displayTopThreeInBoxes(topThree) {
    const loader = document.getElementById('loader-chicken');
    const container = document.getElementById('top-tiles-chicken');
    ...
    topThree.forEach((player, index) => {
        ...
        box.innerHTML = `
            ...
            <span class="chickencoin-icon-small"></span>
                 ${player[3]}
            </div>
            <div class="info-label">PRIZE</div>
            <div class="prize">
            <span class="chickencoin-icon-big"></span>
                 ${player[1]}
            </div>
...
function displayAllParticipants(participants) {
    const table = document.getElementById('sheet-table-chicken');
    ...
    participants.forEach((row, index) => {
        ...
        tr.innerHTML = `
            ...
            alt="Alternative image"/>${row[2]}</td>
                <td><img class="coin" src="/images/chickencoin.svg" />
                ${row[3]}
                </td>
                <td><img class="coin" src="/images/chickencoin.svg" />
                ${row[1]}
...
function startCountdown(targetDate) {
    const countdownElement = document.getElementById("countdown-chicken");
